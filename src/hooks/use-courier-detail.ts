...
const fetchCourierDetail = async (
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch courier details: ${response.statusText}`);
  }
...
export const useCourierDetail = (courierUuid: string) => {
  ...
  return useQuery({
    ...
    queryFn: () => fetchCourierDetail(courierUuid, session?.accessToken),
    enabled: !!session?.accessToken && !!courierUuid, // Only fetch when user is authenticated and UUID is provided
